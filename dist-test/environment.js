 ...
 * ```
 */
export function getEnvVarWithDefault(key, defaultValue) {
    return process.env[key] ?? defaultValue;
}
/**
 ...
 * ```
 */
export function parseEnvNumber(value, defaultValue = 0) {
    if (!value)
        return defaultValue;
 ...
 * ```
 */
export function parseEnvArray(value, delimiter = ',', defaultValue = []) {
    if (!value)
        return defaultValue;
 ...
 * ```
 */
export function getColorSupport() {
    if (!supportsColor()) {
        return 'none';
    }
    const term = process.env.TERM || '';
 ...
 * ```
 */
export function getConfigDirectory(appName = 'juno-task') {
    const platform = getPlatform();
    const home = getHomeDirectory();
    switch (platform) {
        case 'win32':
 ...
 * ```
 */
export function getDataDirectory(appName = 'juno-task') {
    const platform = getPlatform();
    const home = getHomeDirectory();
    switch (platform) {
        case 'win32':
 ...
 * ```
 */
export function getCacheDirectory(appName = 'juno-task') {
    const platform = getPlatform();
    const home = getHomeDirectory();
    switch (platform) {
        case 'win32':
 ...
 * ```
 */
export async function findMCPServerPath(serverName = 'mcp-server') {
    const platform = getPlatform();
    const searchPaths = [];
    ...
    if (platform === 'win32') {
        searchPaths.push('C:\\Program Files\\MCP\\bin', 'C:\\Program Files (x86)\\MCP\\bin', path.join(process.env.LOCALAPPDATA || '', 'Programs', 'MCP', 'bin'));
    }
    else {
        searchPaths.push('/usr/local/bin', '/usr/bin', '/opt/mcp/bin', path.join(getHomeDirectory(), '.local', 'bin'), path.join(getHomeDirectory(), 'bin'));
    }
 ...
 * ```
 */
export async function validateMCPServerPath(serverPath) {
    try {
 ...
 * ```
 */
export function getMCPServerEnvironment(additionalEnv = {}) {
    const baseEnv = {
