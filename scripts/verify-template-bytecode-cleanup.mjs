...
try {
  execFileSync('python3', ['-m', 'py_compile', sourceScript], { cwd: root, stdio: 'pipe' });
  assert.ok((await generatedPaths(sourceCache)).some((name) => name.endsWith('.pyc')),
    'fixture compilation did not create nested __pycache__ bytecode');
  await mkdir(path.dirname(standaloneSource), { recursive: true });
  await writeFile(standaloneSource, 'standalone bytecode fixture\n');
  ...
  assert.deepEqual(await generatedPaths(path.join(root, 'dist/templates/scripts')), [],
    'canonical prepack left Python cache/bytecode in dist template scripts');
  assert.deepEqual(await readFile(sourceScript), sourceBefore,
    'canonical build modified a source template');
  assert.ok((await stat(standaloneSource)).isFile(),
  ...
  await rm(standaloneSource, { force: true });
  await rm(path.dirname(standaloneSource), { recursive: true, force: true });
