...
import { JunoMCPClient, createMCPClient } from '../src/mcp/client.js';
import { MCPTimeoutError, MCPConnectionError } from '../src/mcp/errors.js';
...
describe('MCP Timeout Functionality Validation', () => {
  ...
  describe('Tool Call Timeout Handling', () => {
    ...
    it('should timeout tool calls that exceed the limit', async () => {
      ...
      const mockToolCall = vi.fn().mockImplementation(async () => {
        await new Promise(resolve => setTimeout(resolve, TEST_TIMEOUT + 1000)); // Exceed timeout
