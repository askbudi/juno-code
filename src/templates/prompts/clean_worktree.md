...
6. In a fresh dedicated `yy pi` context with no implicit resume, produce a `pre_merge` PASS receipt bound to the request/PDR, complete base-to-tip diff, expected paths, commits, tests, and open-bug set. Bare `pi`, direct agent/provider CLIs, and indirect model/provider overrides are forbidden. Inherit project defaults or use only an ordinary explicit selector exactly approved by project `workflowModels`; compatibility testing outside that allowlist is separately authorized outside this workflow. A failed review requires a review-fix commit and a fresh receipt; reviewer fixes and acceptance at the resulting exact tip are permitted.
