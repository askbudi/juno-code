...
6. In a fresh dedicated `yy pi` context with no implicit resume, produce a `pre_merge` PASS receipt bound to the request/PDR, complete base-to-tip diff, expected paths, commits, tests, and open-bug set. Bare `pi` and direct provider CLIs are forbidden. Inherit the project provider/model; an override requires explicit owner authorization recorded in the workflow/review evidence. A failed review requires a review-fix commit and a fresh receipt; reviewer fixes and acceptance at the resulting exact tip are permitted.
