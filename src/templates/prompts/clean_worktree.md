...
3. Run `yy task finish TASK_ID` after the worktree is clean and committed. This
   validates affected paths/tests and queues the immutable feature tip.
4. Use `yy merge status` and `yy merge next` to serialize only target mutation.
...
5. Low risk needs no semantic review. Normal risk uses at most one independent
   review. High risk uses Reviewer A then Reviewer B on one frozen candidate.
6. After expected-SHA CAS, verify identity/readback only; do not redispatch a
   semantic reviewer for byte-identical delivery.
7. Cleanup is reachability-safe. Push, release, publish, deploy, production
