...
6. In a fresh dedicated `yy pi` context with no implicit resume, produce a `pre_merge` PASS receipt bound to the request/PDR, complete base-to-tip diff, expected paths, commits, tests, and open-bug set. Bare `pi`, direct agent/provider CLIs, and per-call provider/model overrides are forbidden; inherit the project defaults. Compatibility testing that needs an override is separately authorized outside this workflow. A failed review requires a review-fix commit and a fresh receipt; reviewer fixes and acceptance at the resulting exact tip are permitted.
