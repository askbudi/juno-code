...
8. Integrate only through `integration_owner_preflight.py integrate`, using the verified candidate receipt. It must lock `(Git common directory, full target ref)`, recheck the expected SHA, update the target with compare-and-swap, validate the actual target, and require an `actual_target` PASS receipt. List nested repositories child-first and bind root gitlinks. Partial multi-repository integration is preserved and reported; it is never rewound or called success. Resume only with `--resume-receipt` bound to that exact partial receipt and candidate set.
