...
def canonical_target_config(root: Path, head: str) -> None:
    result = run(["git", "-C", str(root), "show", f"{head}:.juno_task/config.json"], root, False)
    try:
        value = json.loads(result.stdout) if result.returncode == 0 else None
    except json.JSONDecodeError:
        value = None
    if (not isinstance(value, dict) or "lifecycle" in value
            or value.get("controllerWorkspace") != CANONICAL_CONTROLLER_WORKSPACE):
        raise RegistrationError(
            "target controller config does not contain the exact canonical metadata-only workspace shape"
        )
