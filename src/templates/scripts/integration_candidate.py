...
SCHEMA = "juno_integration_candidate.v1"
...
def plan(args:argparse.Namespace)->dict[str,Any]:
    ...
    payload={"schema_version":SCHEMA,"operation":"plan","task_id":args.task_id,"repository":str(repo),"target_ref":target_ref,
      ...
      "premerge_review_sha256":hashlib.sha256(args.premerge_review.read_bytes()).hexdigest(),"pdr_matrix":json.loads(args.pdr_matrix.read_text())}
...
def build(args:argparse.Namespace)->dict[str,Any]:
    ...
    try:
      ...
      payload={**plan_data,"operation":"build","candidate_sha":candidate,"candidate_path":str(candidate_path) if candidate_path else None,
       "candidate_paths":candidate_paths,"validation":validations,"eligible":False,"plan_sha256":hashlib.sha256(args.plan.read_bytes()).hexdigest()}
      write(args.output,payload);return payload
    except Exception:
