...
def main(argv:list[str]|None=None)->int:
 p=argparse.ArgumentParser(description=__doc__,allow_abbrev=False);p.add_argument("integrate",nargs="?");p.add_argument("--repository",action="append",type=parse_repo,required=True);p.add_argument("--candidate-receipt",action="append",type=Path,required=True);p.add_argument("--resume-receipt",type=Path);p.add_argument("--gitlink",action="append",type=parse_gitlink,default=[]);p.add_argument("--controller-checkout",type=Path);p.add_argument("--restore-controller-checkout",choices=("exact_integrated",));p.add_argument("--checked-out-target",choices=("detach_same_sha",));p.add_argument("--risk-tier",choices=tuple(RISK_ORDER),default="high");p.add_argument("--feature-tag",action="store_true");p.add_argument("--require-actual-review",action="store_true",help="require delivery-sensitive post-CAS semantic review without changing objective risk");p.add_argument("--actual-review-command");p.add_argument("--actual-review-receipt",type=Path);p.add_argument("--validation-command",action="append",required=True);p.add_argument("--validation-timeout",type=float,default=3600);p.add_argument("--lock-timeout",type=float,default=30);p.add_argument("--task-id",required=True);p.add_argument("--output",type=Path,required=True);p.add_argument("--inject-failure-after",type=int);p.add_argument("--inject-restoration-failure-after",type=int);p.add_argument("--inject-authority-failure-after",type=int)
 a=p.parse_args(argv)
 ...
 try:
  ...
  actual_required=a.require_actual_review or effective in {"high","release"}
  actual_review_reasons=(["delivery_sensitive"] if a.require_actual_review else [])+(["effective_risk_tier"] if effective in {"high","release"} else [])
  receipt.update({"repositories":plan,"committed_tree_admission":committed_admission,"candidate_receipt_sha256":candidate_hash,"topology":topology,"declared_risk_tier":a.risk_tier,"effective_risk_tier":effective,"risk_escalation_reasons":reasons,"actual_review_required":actual_required,"actual_review_reasons":actual_review_reasons})
