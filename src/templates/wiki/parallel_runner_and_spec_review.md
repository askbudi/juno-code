...
**Reviewer launcher identity:** Launch every project subagent and independent reviewer through `yy pi`, inheriting the project-configured provider/model. Bare `pi`, direct agent/provider CLIs, and per-call provider/model overrides bypass Juno policy and are forbidden in managed workflows/reviews; compatibility testing that needs an override is a separately authorized operation outside this contract. Workflow lint rejects those bypasses plus non-`yy pi` local-integration and actual-target reviewers. Failure mode prevented: a green review has irreproducible launch identity. Runtime contract: context separation does not change launcher ownership. Validation: workflow-runner tests accept default `yy pi`, reject every bypass, and prove runtime/template parity; backing lint matters because prose did not prevent repeated bare launches.
