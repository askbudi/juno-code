...
Low risk uses zero reviews; normal risk permits zero or one; high risk durably enters `AWAITING_RISK` and requires `yy merge review TASK_ID` to run the configured full validation and Reviewer A then Reviewer B against the same frozen SHA. Review only writes a bounded canonical receipt; a later `yy merge next` freshly verifies it and performs CAS. Findings and reviewer failures preserve truthful awaiting state and never move the target. Release authority is separate: a release candidate remains `AWAITING_RELEASE` until an owner-authorized release-gate receipt exists, and the merge queue performs no release action. The full target ref must be unowned by every worktree (use a detached integration checkout); the queue fails closed before CAS rather than making a checked-out branch incoherent. It never rebases, squashes, force-updates, pushes, releases, or synchronizes the controller.
