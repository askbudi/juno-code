...
The helper hashes process commands in receipts, excludes its caller ancestry, never signals processes, and rejects another classified write-capable same-repository process. Process discovery is supplemental because host inspection is platform-sensitive; the hard boundary is cooperative Git-common-dir leases plus clean, exact, stable ref/status evidence. Any integration workflow capable of target mutation must use the same lease helper rather than inventing task-local lock directories.
