...
Git hook adoption is explicit. `controller_checkpoint.py hook install` refuses any existing pre-commit hook until the operator supplies its exact path and SHA-256 with `--approve-existing PATH=SHA256`; the managed guard runs first, then invokes the approved bytes only after rechecking that exact composed sidecar path and hash. Install/status/remove are idempotent and refuse dispatcher, metadata, or composed-byte drift, missing metadata, or sidecar collisions without changing hooks. Refusals never reset, restore, stage, commit, tag, or move refs/worktrees.
