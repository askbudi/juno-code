...
async function executeInitCommandWithArgs(
  ...
  const { timeout = INIT_COMMAND_TIMEOUT, cwd = tempDir, env = {} } = options;
...
async function analyzeFileSystemChanges(
  ...
  const additionalFiles = filesCreated.filter(
    (f) => !f.startsWith('.juno_task/') && f !== '.juno_task',
  );
...
describe('Init Command Execution Tests', () => {
  ...
  it(
    'should execute init command with command line arguments and verify file creation',
    async () => {
      ...
      const commandArgs = [
        'init',
        '--task',
        'Count number of folders in this directory and give me a report',
        '--subagent',
        'codex',
        '--git-url',
      ...
      try {
        ...
        if (result.exitCode === 0 && fileSystemAnalysis.requiredFiles.initMd) {
          ...
          const hasCorrectTask = initContent.includes(
            'Count number of folders in this directory and give me a report',
          );
          const hasCorrectSubagent = initContent.includes('**Preferred Subagent**: codex');
          const hasCorrectGitUrl = initContent.includes(
            'https://github.com/askbudi/temp-test-ts-repo',
          );
