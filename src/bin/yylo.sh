...
route_registered_product_control() {
    ...
    case "$operation:$PREBOOTSTRAP_SUBCOMMAND" in
        ...
        task:start|task:run|task:recover-predispatch|task:hydrate|task:finish|task:checkpoint|task:child-checkpoint|task:sync|task:recovery-authorize|task:recovery-apply|task:runtime-bootstrap|task:lease-heartbeat|task:lease-handoff|task:lease-successor|task:lease-revoke|task:lease-release) effective_operation=orchestration ;;
