...
Use `workflow_assert.py` for named machine-readable diagnostics. Local integration uses three installed helpers: `worktree_lifecycle.py` creates/verifies exact-base named trees and performs typed safe cleanup; `integration_candidate.py target-preflight` read-only classifies the official target as exact, advanced descendant, invalid history, or missing without weakening the task worktree's exact approved base; `integration_candidate.py` then plans/builds direct or both-parent candidates and requires pre-merge plus candidate review receipts; `integration_owner_preflight.py integrate` locks each `(Git common directory, full target ref)` channel, performs exact expected-SHA CAS, validates/reviews the actual target, and creates the local annotated `juno-feature/<task>/<sha>` tag. Controller checkpoint commits use the same channel lock. An explicitly eligible self-target transaction may pass `--controller-checkout PATH --restore-controller-checkout exact_integrated`; after review and before unlocking, it restores initialized nested checkouts child-first to exact candidate gitlinks, then attaches the controller root to the exact integrated branch. Restore failures remain typed, resumable, non-rewinding partial integrations; ordinary product owners stay detached. Target preflight, candidate planning, and integration independently apply the shared committed-tree classifier so `git commit --no-verify` cannot cross into promotion. Successful protected integration internally establishes or advances persisted integration-owner base authority without permitting ordinary owner commits; required feature tags are withheld until that persistence succeeds. Successful exact worktree creation internally establishes task authority before publishing its immutable receipt. The resolver exposes no public task or integration-owner assignment, and coherent caller-supplied receipts confer no role authority. Target movement requires candidate rebuild and re-review. Multi-repository updates are child-first and later failure is recorded as `partial_local_integration` without rewind.

A `local_integration` workflow sets `integration_policy.queue: automatic_after_review_pass`, `channel_scope: git_common_dir_and_target_ref`, and `target_movement: rebuild_and_rereview`. It also declares `orchestration_workspace: controller` (or an absolute external workspace), so semantic review never runs with an immutable candidate as cwd. Non-noop candidate review declares the exact canonical Git worktree top-level in `candidate_read_only.path` plus its full SHA. Workflow Runner proves review cwd is external, compares HEAD, worktree/common-dir identity, and bounded hashes/byte counts for raw/logical index and complete status around dispatch, then hash-binds guard evidence into checkpoints and immutable attempt archives. Recovery/amendment revalidate that evidence and current identity; mutation, deletion, replacement, substitution, or tampering fails without cleanup. Its integration step uses an argv-list command that directly executes `integration_owner_preflight.py integrate` (optionally via `python3`), never a shell string or wrapper, so Workflow Runner grants nested-review evidence capability only to the owner process. Same-channel jobs serialize while disjoint channels proceed independently. Feature tags are local integration evidence; `vX.Y.Z`, push, publication, release, deployment, and post-deploy E2E each require separate authority.
